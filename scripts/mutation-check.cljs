...
(defn- tmpfile [suffix]
  (path/join (os/tmpdir) (str "meti-mutation-" suffix ".edn")))
...
(def laws-only
  ...
  register cites no page.

  The law branch is tested against THIS, because the e-Gov API is not the part
  that pushes back: it answered twenty-two consecutive lookups while this
  register was built, whereas every agency host here serves a bot challenge
  when asked too often. Testing the law branch against a page-bearing register
  would put four needless agency requests behind each mutation and, measured
...
(def with-pages
  ...
  branches."
  (subset #{"host.www-chusho-meti-go-jp" "host.www-enecho-meti-go-jp"
            "law.fefta" "chusho.home" "enecho.home" "enecho.basic-plan"}))
...
(def structural
  [{:id "coverage-entry-count"
   ...
   {:id "law-entry-without-law-id"
    ...
    :mutate #(alter-entity % "law.fefta" (fn [e] (dissoc e :egov/law-id)))
...
(def network
  [{:id "nonexistent-law" :on :laws-only
    ...
    :mutate #(alter-entity % "law.fefta" (fn [e] (assoc e :egov/law-id "999ZZ9999999999")))
    :want-exit 1 :want-reason "nonexistent-law"}

   {:id "law-title-drift" :on :laws-only
    :why "a real id recorded under the wrong title"
    :mutate #(alter-entity % "law.fefta" (fn [e] (assoc e :egov/law-title "外国為替法")))
   ...
   {:id "missing-text"
    ...
    :mutate #(alter-entity % "enecho.basic-plan"
